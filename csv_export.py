...
def _atomic_csv_write(output_path: Path, write_rows) -> None:
    """Ecrit un CSV de maniere atomique : les lignes sont d'abord ecrites
    dans un fichier temporaire cree a COTE de la destination finale (meme
    dossier, donc meme systeme de fichiers - condition necessaire pour que
    os.replace() soit atomique), puis ce fichier est renomme vers le chemin
    final uniquement une fois l'ecriture entierement terminee avec succes.

    Auparavant, les deux fonctions d'export ecrivaient directement dans le
    fichier final ligne par ligne : si une erreur survenait en cours de
    route (disque plein, permission revoquee, cle USB retiree...), un CSV
    TRONQUE (en-tete + quelques lignes seulement) restait sur le disque a
    l'emplacement final. L'utilisateur etait bien informe de l'echec (voir
    gui.py, messagebox.showerror), mais ce fichier partiel residuel
    pouvait etre confondu avec un export complet si le message d'erreur
    etait ignore ou ferme trop vite (bug trouve a l'audit, voir C8). En cas
    d'echec, le fichier temporaire est supprime : jamais de fichier partiel
    au chemin final, jamais de fichier orphelin non plus."""
    ...
    try:
        ...
        try:
            ...
            pass  # rien de plus a faire si meme le nettoyage echoue
        raise
...
def export_time_entries_csv(entries: list, output_path: Path) -> None:
    ...
    _atomic_csv_write(Path(output_path), _write_rows)
